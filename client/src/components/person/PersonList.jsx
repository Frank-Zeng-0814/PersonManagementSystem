import { Edit, Trash2, Upload, User } from 'lucide-react';
import { useState } from 'react';
import axios from 'axios';
import toast from 'react-hot-toast';

const PersonList = ({ peopleList, onPersonEdit, onPersonDelete, onAvatarUpdate }) => {
    ...
    const handleAvatarUpload = async (personId, file) => {
        if (!file) return;
        ...
        try {
            ...
            const response = await axios.post(`${BASE_URL}/${personId}/upload-avatar`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });

            toast.success('Avatar uploaded successfully!');
            onAvatarUpdate(personId, response.data.avatarUrl);
        } catch (error) {
            console.error(error);
                                ...
                                <div className="flex items-center space-x-3">
                                    ...
                                    <label className="cursor-pointer">
                                        <input
                                            ...
                                            accept="image/*"
                                            className="hidden"
                                            onChange={(e) => handleAvatarUpload(person.id, e.target.files[0])}
                                            disabled={uploadingId === person.id}
                                        />
                                        <div className="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded hover:bg-gray-200 transition-colors">
                                            <Upload className="w-3 h-3 mr-1" />
                                            {uploadingId === person.id ? 'Uploading...' : 'Upload'}
                                        </div>
                                    </label>
                                </div>
                            </td>
